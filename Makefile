...
ifeq ($(OS),Windows_NT)
	...
	@powershell -NoProfile -ExecutionPolicy Bypass -Command "$$ErrorActionPreference = 'Stop'; New-Item -ItemType Directory -Force -Path bin | Out-Null; $$json = Invoke-RestMethod -Uri 'https://api.github.com/repos/router-for-me/CLIProxyAPI/releases/latest'; $$version = $$json.tag_name; $$versionNum = $$version.TrimStart('v'); $$asset = \"CLIProxyAPI_$${versionNum}_$(CLIPROXY_ARCH).$(CLIPROXY_EXT)\"; $$url = \"https://github.com/router-for-me/CLIProxyAPI/releases/download/$${version}/$${asset}\"; Write-Host \"Downloading $${url}\"; $$archive = \"bin/cliproxy-archive.$(CLIPROXY_EXT)\"; Invoke-WebRequest -Uri $${url} -OutFile $${archive}; Expand-Archive -Path $${archive} -DestinationPath bin -Force; Remove-Item -Force $${archive}; Remove-Item -Force -ErrorAction SilentlyContinue bin/README*.md, bin/README_CN.md, bin/README_JA.md, bin/LICENSE, bin/config.example.yaml; Write-Host 'Done. Binary at bin/cli-proxy-api.exe'"
else
	@echo "Downloading CLIProxyAPI..."
	@mkdir -p bin
	@VERSION=$$(curl -sI https://github.com/router-for-me/CLIProxyAPI/releases/latest | grep -i '^location:' | sed 's/.*tag\///' | tr -d '\r\n'); \
...
ifeq ($(OS),Windows_NT)
	...
	if [ -f bin/cli-proxy-api ]; then \
		CURRENT=$$(./bin/cli-proxy-api 2>&1 | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+(-[0-9]+)?' | head -1 || echo "0.0.0"); \
		...
		echo "CLIProxyAPI not installed."; \
	fi; \
...
update-and-run: update-cliproxy build
	@echo "Starting CLIProxyAPI on :8318 and ThinkingProxy on :8317"
...
auth-claude:
	./bin/cli-proxy-api -config config/cliproxy.yaml -claude-login
